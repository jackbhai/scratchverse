...
if (!process.env.SV_SHOTS_READY) {
  ...
  const r = spawnSync(process.execPath, [fileURLToPath(import.meta.url)], {
    ...
    env: { ...process.env, SV_SHOTS_READY: '1',
      LD_LIBRARY_PATH: `${ROOT}/.browser-libs:${process.env.LD_LIBRARY_PATH || ''}`,
      FONTCONFIG_PATH: `${ROOT}/.browser-libs`, FONTCONFIG_FILE: `${ROOT}/.browser-libs/fonts.conf` },
  });
...
function baseSeed() {
  ...
  s.balance = 2_400_000; s.jp = 26; s.tokens = 9; s.level = 9; s.xp = 40;
  ...
  s.stats = { ...s.stats, scratched: 1420, wins: 402, losses: 1018, spent: 6.1e6, earned: 9.4e6,
    supers: 1, bestWin: 4.86e5, streak: 2, bestStreak: 9, plates: 410, refunds: 51, jackpots: 18 };
  ...
  s.skins = { gold: true, rose: true, neon: true, carbon: false };
  s.matsOwned = { wood: true, felt: true, gems: true };
  s.skin = 'gold'; s.matBg = 'wood';
...
const rollWinning = (s, id, tries = 4000) => {
  ...
  for (let i = 0; i < tries; i++) { const t = rollTicket(s, T); if (t.win && t.payout >= T.price * 2) return t; }
  return rollTicket(s, T);
};

/* A — manual play: table empty → buy → tap winner from tray → scratch → claim */
const sA = baseSeed();
sA.gadgets = { ...sA.gadgets, bot: { lvl: 3, on: false }, fan: { lvl: 2, on: false }, mundo: { lvl: 1, on: false }, auto: { lvl: 1, on: false } };
sA.table = null;
...
sA.mat = [{ ...superT, id: 'seedsuper', super: true, done: true, settled: true, status: 'mat',
  payout: Math.round(60_000_000 * 41), scratch: Array(9).fill(1), revealed: 9 }];
sA.feed = [
...
const sB = baseSeed();
sB.balance = 148e6; sB.lifetime = { earn: 6.4e7, spent: 5.1e7, jp: 26 }; sB.run = { earn: 3.2e7, spent: 2.9e7, peak: 4.4e7 };
sB.gadgets = { ...sB.gadgets, bot: { lvl: 4, on: true }, fan: { lvl: 2, on: true }, mundo: { lvl: 1, on: true }, auto: { lvl: 2, on: true }, egg: { lvl: 2, until: 0, readyAt: 0 } };
sB.settings = { ...sB.settings, autoClaim: true };
sB.table = { ...rollTicket(sB, TICKETS.find((t) => t.id === 'appletree')), status: 'table' };
sB.tray = Array.from({ length: 7 }, () => ({ ...rollTicket(sB, TICKETS.find((t) => t.id === 'quickcash')), status: 'tray' }));
const SEEDB = JSON.stringify(sB);

/* ---------------- boot ---------------- */
const server = spawn('npx', ['vite', '--host', '127.0.0.1', '--port', String(PORT), '--strictPort'],
  { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'], env: { ...process.env } });
const bye = () => { try { server.kill('SIGTERM'); } catch {} };
process.on('exit', bye);
const wait = (ms) => new Promise((r) => setTimeout(r, ms));
const up = async () => { for (let i = 0; i < 240; i++) { try { if ((await fetch(URL)).ok) return true; } catch {} await wait(250); } return false; };
if (!await up()) { console.error('dev server did not start'); bye(); process.exit(1); }

const b = await chromium.launch({ args: ['--no-sandbox', '--font-render-hinting=none', '--force-color-profile=srgb'] });
const ctx = await b.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 3, isMobile: true, hasTouch: true, reducedMotion: 'no-preference' });
const page = await ctx.newPage();
const errs = [];
const IGNORE = /net::ERR_(CONNECTION_REFUSED|FAILED)|WebSocket connection to|favicon/i;
page.on('console', (m) => { const t = m.text(); if (m.type() === 'error' && !IGNORE.test(t)) errs.push('console: ' + t.slice(0, 200)); });
page.on('pageerror', (e) => { errs.push('pageerror: ' + String(e).slice(0, 200)); log('   PAGEERR: ' + String(e).slice(0, 200)); });
const log = (...a) => console.log(...a);
const shot = async (n, sel) => {
  await page.evaluate(() => { document.querySelectorAll('.main').forEach((e) => { e.scrollTop = 0; }); window.scrollTo(0, 0); }).catch(() => {});
  await wait(320);
  try { sel ? await page.locator(sel).first().screenshot({ path: `${OUT}/${n}.png` }) : await page.screenshot({ path: `${OUT}/${n}.png` }); }
  catch { await page.screenshot({ path: `${OUT}/${n}.png` }); }
  log(`  ✓ ${n}`);
};
const st = (msg) => page.evaluate(() => ({
  screen: document.querySelector('.screen h2')?.textContent,
  card: !!document.querySelector('#scratchCv'),
  pills: [...document.querySelectorAll('.pill .v')].map((x) => x.textContent),
...
const scratch = (rows, per = 7) => page.evaluate(async ([rows, per]) => {
  ...
  const fire = (t, x, y) => cv.dispatchEvent(new PointerEvent(t, {
    pointerId: 1, isPrimary: true, pointerType: 'touch', clientX: x, clientY: y,
    bubbles: true, cancelable: true, buttons: t === 'pointerup' ? 0 : 1,
  }));
  for (const [fx1, fy1, fx2, fy2] of rows) {
    const [x1, y1] = P(fx1, fy1), [x2, y2] = P(fx2, fy2);
...
const serp = (n, top = 0.06, bot = 0.94, vert = false) =>
  Array.from({ length: n }, (_, i) => {
    ...
    return vert
      ? (i % 2 ? [a, 0.01, a, 0.99] : [a, 0.99, a, 0.01])
      : (i % 2 ? [0.01, a, 0.99, a] : [0.99, a, 0.01, a]);
  });
const hatch = (n = 7) => [...serp(n, 0.05, 0.95), ...serp(n, 0.05, 0.95, true)];

/* ============ 01 onboarding (cold save in an isolated context) ============ */
if (ORGANIC) {
  await page.goto(URL, { waitUntil: 'load' }); await wait(2000);
  await shot('01-onboarding');
  await page.getByText(/Start with/).click(); await wait(1400);
} else {
  const coldCtx = await b.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 3, isMobile: true, hasTouch: true });
...
await page.locator('.tab', { hasText: 'Shop' }).click(); await wait(1100);
await shot('04-catalog');
await page.locator('.tk button', { hasText: /odds/i }).nth(1).click(); await wait(800);
await shot('05-odds-sheet');
await page.locator('.sheet .btn.p, .sheet .btn').last().click().catch(() => {});
await wait(500);
await page.keyboard.press('Escape').catch(() => {});
await page.locator('.tab', { hasText: 'Table' }).click(); await wait(900);
...
await page.locator('.tray .mini-t').first().click({ timeout: 6000 }).catch(() => {});
await wait(900);
...
await page.locator('.tcard').click({ position: { x: 30, y: 420 } }).catch(() => {});
await page.getByRole('button', { name: /claim/i }).first().click({ timeout: 4000 }).catch(() => {});
await wait(1100);
await shot('08-claimed');
await st('claimed');

/* ============ 09 sticky mat with the parked SUPER JACKPOT ============ */
await page.locator('.tray .sticky, .mini-t.sticky').first().click({ timeout: 5000 })
  .catch(() => log('   (mat ticket is on the table after tap)'));
await wait(900);
await shot('09-sticky-mat');

/* ============ 10 bots tab ============ */
await page.locator('.tab', { hasText: 'Bots' }).click(); await wait(1000);
await shot('10-bots');

/* ============ 11 prestige, 12 profile, 13 night market ============ */
await page.locator('.tab', { hasText: 'JP' }).click(); await wait(900);
await shot('11-prestige');
await page.locator('.tab', { hasText: 'You' }).click(); await wait(900);
await shot('12-profile');
const nm = page.locator('button, .h3, h2').filter({ hasText: /night market/i }).first();
if (await nm.count()) { await nm.click().catch(() => {}); await wait(800); await shot('13-night-market'); }
...
await pb.locator('.tab', { hasText: 'Bots' }).click(); await pb.waitForTimeout(900);
await pb.screenshot({ path: `${OUT}/15-bots-stage.png` });
log('  ✓ 15-bots-stage');
await ctxB.close();

/* ============ 16 the card itself, close-up ============ */
await page.locator('.tab', { hasText: 'Table' }).click(); await wait(600);
for (let i = 0; i < 4 && !(await page.locator('#scratchCv').count()); i++) {
  await page.locator('.tab', { hasText: 'Shop' }).click(); await wait(420);
  await page.locator('.tk button:not([disabled])', { hasText: /buy/i }).first().click({ timeout: 6000 }).catch(() => {});
  await wait(420);
  await page.locator('.tab', { hasText: 'Table' }).click(); await wait(900);
}
...
await page.setViewportSize({ width: 360, height: 800 }); await wait(700);
...
if (!(await pc.locator('#scratchCv').count())) {
  await pc.locator('.tab', { hasText: 'Shop' }).click(); await pc.waitForTimeout(500);
  await pc.locator('.tk button:not([disabled])', { hasText: /buy/i }).first().click({ timeout: 8000 }).catch(() => {});
  await pc.locator('.tab', { hasText: 'Table' }).click(); await pc.waitForTimeout(900);
}
const cb = await pc.locator('#scratchCv').boundingBox().catch(() => null);
if (cb) {
...
log(errs.length ? `⚠ ${errs.length} app console error(s):\n` + errs.slice(0, 8).join('\n') : '✓ no app console/page errors (HMR noise ignored)');
await b.close();
bye();
const { spawnSync } = await import('node:child_process');
