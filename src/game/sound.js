...
let AC = null, master = null, noiseBuf = null, vibrate = true, enabled = true;
let lastScratch = 0;
...
function tone({ f = 440, to = f, type = 'sine', dur = 0.14, g = 0.16, delay = 0, q = 0 }) {
  const ac = ctx(); if (!ac || !enabled) return;
...
const buzz = (p) => { if (vibrate && navigator.vibrate) { try { navigator.vibrate(p); } catch {} } };
...
const _SFX = {
  ...
  click() { tone({ f: 520, to: 300, type: 'triangle', dur: 0.07, g: 0.08 }); buzz(6); },
  buy() { tone({ f: 700, to: 1100, type: 'square', dur: 0.09, g: 0.05 }); noise({ dur: 0.12, f: 900, g: 0.06 }); buzz(8); },
  scratch(v = 0.5) {
  ...
  reveal() { tone({ f: 900, to: 1500, type: 'sine', dur: 0.09, g: 0.07 }); buzz(5); },
  coin(n = 1) {
  ...
  win(mult = 1) {
    ...
    seq.forEach((f, i) => tone({ f, to: f * 1.01, type: 'triangle', dur: 0.2, g: 0.11, delay: i * 0.06 }));
    noise({ dur: 0.5, g: 0.05, f: 5200, rate: 1.4, delay: 0.05 });
    buzz([12, 40, 18, 40, 30]);
  },
  lose() { tone({ f: 260, to: 120, type: 'sawtooth', dur: 0.24, g: 0.05 }); buzz(18); },
  jackpot() {
    const seq = [523, 659, 784, 1046, 1318, 1568, 2093];
    seq.forEach((f, i) => tone({ f, type: 'square', dur: 0.3, g: 0.075, delay: i * 0.07 }));
  ...
  level() { [660, 880, 1170].forEach((f, i) => tone({ f, dur: 0.14, g: 0.08, type: 'triangle', delay: i * 0.07 })); buzz([10, 30, 10]); },
  break_() { for (let i = 0; i < 5; i++) noise({ dur: 0.08, g: 0.09, f: 2200 + i * 900, rate: 1.9, delay: i * 0.02 }); tone({ f: 300, to: 90, type: 'sawtooth', dur: 0.2, g: 0.05 }); buzz([30, 20, 40]); },
  whoosh() { noise({ dur: 0.4, g: 0.07, f: 700, rate: 0.5 }); },
  prestige() { [196, 262, 330, 392, 523, 659].forEach((f, i) => tone({ f, dur: 0.5, g: 0.08, type: 'sine', delay: i * 0.1 })); buzz([40, 60, 40, 60, 120]); },
  error() { tone({ f: 220, to: 180, type: 'square', dur: 0.12, g: 0.05 }); },
};
