...
// ============================================================
import {
  COINS, UPGRADES, GADGETS, ACHIEVEMENTS, PITY, AUTO_AT,
  HARDNESS_DAB, xpNeed, jpFrom, nodeCost, MONEY_CAP, unlockGate,
} from './config.js';
...
export function expectedMatches(s, ticket, i) {
...
export function rollTicket(s, ticket) {
  ...
  if (win) {
    ...
    syIdx = good[0].i;
    for (const g of good) { r -= g.x.w * (1 + (g.x.pay > 0 ? st.luckPct : 0) / 130); if (r <= 0) { syIdx = g.i; break; } }
  } else {
...
export function applyDab(t, ticket, cx, cy, r, strength) {
  ...
  for (let i = 0; i < CELLS; i++) {
    const gx = (i % 3 + 0.5) / 3, gy = (Math.floor(i / 3) + 0.5) / 3;
...
export function applyHazards(s, ticket, t) {
  ...
  const snake = ticket.syms.findIndex((x) => x.e === '🐍');
  if (!hit && snake >= 0) {
...
export function payoutFor(s, ticket, t) {
  ...
  if (t.final) {
    return { pay: Math.round(Math.min(MONEY_CAP, Math.max(s.balance * 3, ticket.price * 12))), penalty: 0, superWin: false, jackpot: false, final: true };
  }
...
export const foilFor = (s, ticket) => {
  const skin = s.skins?.[s.skin] !== false && s.skin ? s.skin : null;
  if (skin && skin !== 'gold' && s.skins?.[skin]) return ({ gold: 'foil-gold', rose: 'foil-rose', neon: 'foil-neon', carbon: 'foil-carbon' })[skin] || 'foil-gold';
  return 'foil-gold';
};
...
export function checkAchievements(s) {
  ...
  for (const a of ACHIEVEMENTS) {
    ...
    let ok = false;
    try { ok = !!a.test(s); } catch { ok = false; }
    if (ok) { s.achievements[a.id] = Date.now(); s.tokens += a.tok; got.push(a); }
  }
  return got;
}
...
export function gadgetValue(s, id) {
  ...
  return { g, lvl, max: g?.max || 0, cost: lvl >= g.max ? null : Math.round(g.base * Math.pow(g.k, lvl)), jpGate: (g?.gateJP || 0) > (s.jp || 0) ? g.gateJP : 0 };
}
...
export const pay = (s, price) => { s.balance = Math.max(0, (s.balance || 0) - price); };
