...
import {
  TICKET_BY_ID, SKINS, MATS, GADGETS, JP_NODES,
  COINS, ENDINGS, XP_PER_LEVEL, PRESTIGE_BASE, MONEY_CAP,
  platePay, plateBreak,
} from './game/config.js';
import {
  stats, rollTicket, applyHazards, payoutFor, tossRefund, addXp, checkAchievements,
  jpEarnable, isUnlocked, upgradeValue, gadgetValue, nodeValue, revealCells,
  tableSlots, traySlots, botSpeed, isOn, claimInterval, autoInterval,
  dailyReward, nid,
} from './game/logic.js';
import { loadState, persist, backup, initialState, todayKey, merge } from './db/store.js';
import SFX from './game/sound.js';

const clampBal = (n) => Math.max(0, Math.min(MONEY_CAP, Math.round(n || 0)));
const q = (s, item) => ({ ...s, queue: [...(s.queue || []), { id: nid('q'), ...item }] });
...
function settle(s, id, opts = {}) {
  ...
  if (opts.toss) {
    ...
    next.feed = [{ e: '🗑️', x: `${ticket.name} tossed · +${refund}`, a: refund, at: Date.now() }, ...(s.feed || [])].slice(0, 14);
  ...
  const st = { ...s.stats };
  st.earned += res.pay; st.wins += win ? 1 : 0; st.losses += win ? 0 : 1;
  ...
  if (isFinal) {
    ...
    next.tray = (s.tray || []).filter((x) => x.id !== id);
    next = q(next, { t: 'phone' });
    next.feed = [{ e: '☎️', x: 'The Corporation is calling…', at: Date.now() }, ...(next.feed || [])].slice(0, 14);
  ...
  if (s.table?.id === id) next.table = null;
  next.mat = park ? [done, ...(s.mat || []).filter((x) => x.id !== id)].slice(0, 3)
                  : (s.mat || []).filter((x) => x.id !== id);
  next.tray = (s.tray || []).filter((x) => x.id !== id);
  ...
  if (win) {
    ...
    next.feed = [
      { e: isSuper ? '✨' : res.jackpot ? '🎯' : '🪙', x: `${ticket.name} ${isSuper ? 'SUPER JACKPOT' : res.jackpot ? 'jackpot' : 'paid'} +${res.pay}`, a: res.pay, at: Date.now() },
      ...(next.feed || [])].slice(0, 14);
    ...
    return q(next,
      ...
      ...(isSuper ? [{ t: 'sound', v: 'jackpot' }, { t: 'rain', v: 30 }] : []),
      ...(evs.map((e) => ({ t: 'level', v: e.v, bonus: e.bonus }))),
      { t: 'sound', v: isSuper ? 'jackpot' : ratio >= 6 ? 'jackpot' : 'win' });
  }

  next.feed = [{ e: res.penalty ? '💀' : '🫥', x: `${ticket.name} ${res.penalty ? 'bit you' : 'paid nothing'} −${ticket.price}`, r: res.penalty || ticket.price, at: Date.now() }, ...(next.feed || [])].slice(0, 14);
  const got = checkAchievements(next);
  next = markAchieved(next, got);
  return q(next, { t: 'sound', v: 'lose' }, { t: 'toast', text: res.penalty ? `Hazard! −${res.penalty + ticket.price}` : 'No win', kind: 'bad' });
}
...
function buy(s, ticketId, n = 1, opts = {}) {
  ...
  if (s.balance < total) { SFX.error(); return q(s, { t: 'toast', text: 'Not enough coins', kind: 'bad' }); }
...
function tick(s, now) {
  ...
  if (isOn(next, 'bot') && next.table && !next.table.done && !(next.table.claimAt && !isOn(next, 'mundo'))) {
    ...
    const n = Math.max(1, Math.floor(want) - (t.revealed || 0));
    {
      ...
      if (!next.table && next.tableQueue?.length) {         // next ticket off the fan
  ...
  if (isOn(next, 'mundo')) {
    ...
    for (const p of parked) {
      if (now - (p.createdAt || 0) > 900) { next = settle(next, p.id); changed = true; break; }
    }
  }
  ...
  if (now - (next._last?.pool || 0) > 4000 && (next.run.spent || 0) > 0) {
    ...
    for (const k of Object.keys(pools)) {
      ...
      if (add > 1) { pools[k] = Math.round(pools[k] + add); moved = true; }
    }
    next = { ...next, _last: { ...next._last, pool: now } };
    if (moved) { next.pools = pools; changed = true; }
  }
...
export function reducer(s, a) {
  ...
  switch (a.type) {
    ...
    case 'QUEUE_POP': return { ...s, queue: (s.queue || []).filter((x) => x.id !== a.id) };
    case 'UI': return { ...s, ui: merge(s.ui || {}, a.patch) };
    case 'SETTINGS': {
      const settings = { ...s.settings, ...a.patch };
      SFX.settings(settings);
      return { ...s, settings };
    }
    case 'NAME': return { ...s, name: String(a.name || 'Maxed').slice(0, 12) };
    case 'ONBOARD': return { ...s, seenOnboard: true, name: a.name || s.name, balance: clampBal(s.balance + (a.bonus || 0)) };
    case 'COIN': {
      const c = COINS.find((x) => x.id === a.id);
      if (!c) return s;
      if ((s.lifetime.earn || 0) < c.gate) return q(s, { t: 'toast', text: `Locked until ${c.gate.toLocaleString()} lifetime earnings`, kind: 'bad' });
      return { ...s, coin: a.id };
    }
    case 'BUY': return buy(s, a.ticket, a.n || 1);
    case 'SELECT': {
      if (!a.ticket) { // pull from tray to table
        const t = (s.tray || [])[a.index ?? 0];
        if (!t) return s;
        return { ...s, table: s.table ? { ...s.table, status: 'tray', ...{} } : { ...t, status: 'table' }, tray: s.table ? [{ ...s.table, status: 'tray' }, ...s.tray.filter((x) => x.id !== t.id)] : s.tray.filter((x) => x.id !== t.id) };
      }
    ...
    case 'FINISH': return s.table && !s.table.settled ? settle(s, s.table.id) : s;
    case 'REVEAL_ALL': {
      if (!s.table || s.table.done) return s;
      const st = stats(s);
    ...
    case 'TAB': return { ...s, ui: { ...s.ui, tab: a.tab }, settings: { ...s.settings, tab: a.tab } };
    case 'CLAIM': {
    ...
    case 'BUY_UPG': {
      ...
      if (!u) return s;
      if (lvl >= max) return s;
      if (s.balance < cost) { SFX.error(); return q(s, { t: 'toast', text: 'Not enough coins', kind: 'bad' }); }
    ...
    case 'BUY_GD': {
      ...
      if (s.balance < cost) { SFX.error(); return q(s, { t: 'toast', text: 'Not enough coins', kind: 'bad' }); }
      const next = { ...s, balance: clampBal(s.balance - cost), gadgets: { ...s.gadgets, [a.id]: { ...s.gadgets[a.id], lvl: lvl + 1 } } };
    ...
    case 'EGG': {
      ...
      return q(next, { t: 'sound', v: 'whoosh' }, { t: 'toast', text: 'Cranked! Gadgets ×' + (1.6 + g.lvl * 0.25).toFixed(2), kind: 'good' });
    }
    case 'AUTO_SET': return { ...s, autoTarget: a.ticket || s.autoTarget, autoQty: a.qty ?? s.autoQty, autoReserve: a.reserve ?? s.autoReserve };
    case 'DAILY': {
      const day = todayKey();
      ...
      const next = {
        ...
        daily: { ...s.daily, day, last: day, claimed: true, streak, spell: todayKey(), charges: Math.max(s.daily.charges, 2 + (s.gadgets.spell.lvl || 0)) },
      };
    ...
    case 'PRESTIGE': {
      ...
      if (gain < 1) return q(s, { t: 'toast', text: `Need ${Math.ceil(PRESTIGE_BASE).toLocaleString()} earned this run`, kind: 'bad' });
      const keep = {
        v: 1, name: s.name, level: s.level, xp: s.xp, jp: s.jp + gain, tokens: s.tokens,
        achievements: s.achievements, skins: s.skins, mat: s.mat, skin: s.skin, settings: s.settings,
        endings: s.endings, coin: s.coin, nodes: s.nodes, lifetime: s.lifetime,
        stats: s.stats, runs: (s.runs || 1) + 1, bestBalance: s.bestBalance, createdAt: s.createdAt,
        seenOnboard: true,
      };
      ...
      if (s.nodes.mech) for (const g of GADGETS) if (g.id !== 'machine') fresh.gadgets[g.id] = { ...fresh.gadgets[g.id], lvl: Math.max(1, fresh.gadgets[g.id].lvl) };
    ...
    case 'NODE': {
      ...
      return q({ ...s, jp: s.jp - cost, nodes: { ...s.nodes, [n.id]: lvl + 1 } }, { t: 'sound', v: 'buy' }, { t: 'toast', text: `${n.name} Lv.${lvl + 1}`, kind: 'good' });
    }
    case 'SKIN': {
      ...
      if (!s.skins[a.id]) {
        ...
        const next = { ...s, tokens: s.tokens - sk.tok, balance: clampBal(s.balance - sk.coin), skins: { ...s.skins, [a.id]: true }, skin: a.id };
    ...
    case 'ENDING': {
      ...
      return q(checkA(next, { t: 'sound', v: 'jackpot' }, { t: 'rain', v: 34 }), { t: 'toast', text: `Ending: ${e.name}`, kind: 'good' });
    }
    case 'RESET': {
...
export function GameProvider({ children }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const patched = {
        ...base,
        daily: base.daily?.day === day ? base.daily : { ...base.daily, day, charges: 2 + (base.gadgets?.spell?.lvl || 0), spell: day },
        ui: base.ui && typeof base.ui === 'object' ? { ...base.ui, tab: base.settings?.tab || 'table' } : { tab: 'table', sheet: null },
        queue: [],
        matsOwned: { wood: true, felt: true, ...(base.matsOwned || {}) },
        autoQty: base.autoQty || 1,
      };
      dispatch({ type: 'INIT', state: patched });
      loaded.current = true;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { persist(state); }, 700);
    return () => clearTimeout(t);
  }, [state]);
  useEffect(() => {
    if (!loaded.current || !state) return;
    const i = setInterval(() => backup(state), 90000);
    const onHide = () => backup(state);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => dispatch({ type: 'QUEUE_POP', id: first.id }), first.t === 'win' ? 160 : 90);
    return () => clearTimeout(t);
  }, [state?.queue]);

  const api = useMemo(() => ({
    s: state, dispatch,
    st: state ? stats(state) : null,
  }), [state]);

  if (!state) {
    return React.createElement('div', { className: 'splash' }, [
      React.createElement('img', { key: 'i', src: './assets/logo.png', alt: '' }),
      React.createElement('div', { key: 't', className: 'd', style: { opacity: 0.6, fontSize: 12 } }, 'loading your save…'),
    ]);
  }
