...
> generated by code — and that includes the app's install icons, which `npm run icons` paints from
> the same crest geometry the header uses (the only rasters in the repo, and nothing in the app loads one).
...
npm run test       # vitest units (51) + the bundled engine/mount suite (239 assertions)
...
npm run icons      # re-draw the three PWA install icons from the crest recipe (scripts/build-icons.py)
```
...
- Installable PWA + offline service worker, 404 SPA fallback, CI deploy, and **290 automated
  assertions** (51 vitest units + the 239-assertion engine/mount suite).
