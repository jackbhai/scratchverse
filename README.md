...
> **Zero bitmap art.** Every ticket face, paper seal, coin, prop glyph and table texture in this game is
...
npm run test       # vitest units (51) + the bundled engine/mount suite (237 assertions)
...
| Surfaces | `src/ui/art.jsx` | `TicketFace` draws a 120×176 card from the ticket's own data: engraved guilloché field (deterministic PRNG seeded by ticket id), hairline double frame, catalogue + price rail, motif glyph, name plate. `PAPER` gives each coating a stock + ink colour, `paperCss()` / `PaperSwatch` render the Night Market samples, `MATS_CSS` the five table themes as pure CSS. `Crest` and `Coin` are the app's mark and currency. |
| Motion & chrome | `src/styles.css`, `src/ui/base.jsx` | AMOLED layer stack (`@layer theme, reset, layout, chrome, components, card, effects`), one `<Modal>` for every overlay, WAAPI paper shreds, spring pay-line, pointer-driven gloss and tilt. |

Because the card art is *generated from the data*, adding a ticket to `config.js` gives it a
correct face, stock tint, odds panel and icon row automatically — there is nothing to render,
...
| Achievements (34) → tokens → Night Market cosmetics | 12 achievements → tokens → Night Market: 4 paper stocks + 5 table themes (all vector) |
...
- Installable PWA + offline service worker, 404 SPA fallback, CI deploy, and **288 automated
  assertions** (51 vitest units + the 237-assertion engine/mount suite).

## 5. Architecture

```
src/
  styles.css               @layer'd AMOLED design system + [data-skin] paper-stock re-tint
  ...
  ui/art.jsx               TicketFace / paper stocks / mat themes / crest / coin, all procedural
  ui/base.jsx              Modal (every overlay), IconBtn, Chip, Switch, Bar, Stat, Lv, Swatch
  components/ScratchCard.jsx    canvas paper seal: one touch tears a cell → auto-complete at 55 %
...
1. **The seal is painted, never loaded.** Laid and chain lines, ~420 seeded fibres, a curl gradient,
   a letterpress ticket stamp (`APPLE TREE · CATALOGUE · ONE TOUCH TEARS`), nine dimpled cell wells and
   a deckled silhouette with its own drop shadow — all drawn from the ticket's own data.
2. **One touch tears one cell.** `cellsForStroke()` maps the pointer segment onto the 3×3 grid and
   `tearCells()` takes the paper off those cells whole — no half-rubbed symbols, no digging. A wide
   brush (the Quarter coin rush) also takes the neighbours, which is the only way strength matters now.
3. **The hole is ragged on purpose.** `punch()` erases a jittered polygon, leaves a bright fibre edge,
   a shadow the lifted paper casts on the print, and two or three slivers still hanging by a thread.
   It is deterministic per cell, so a resize or a skin change repaints the exact same tears.
4. Five of nine torn (≥ 55 %) → auto-complete: the seal fades, cells spring open, matched cells get a
   drawn pay-line, the hazard check fires and the payout resolves.
...
6. Shreds are real DOM elements animated with WAAPI (`element.animate`), capped at 26 live bits and
   skipped entirely under `reduceFx` — and where WAAPI is missing, scratching still works.
